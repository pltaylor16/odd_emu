...
# --- Load merged dataset ---
data = np.load('/srv/scratch3/taylor.4264/odd_emu/production_run_bacco/merged/logpk_data.npz')
...
class RHS(eqx.Module):
    ...
    def __init__(self, key):
        self.mlp = eqx.nn.MLP(in_size=nk+2, out_size=nk, width_size=512, depth=4, key=key)

    def __call__(self, P, H, rho):
        return self.mlp(jnp.concatenate([P, H, rho]))

@eqx.filter_value_and_grad
def loss_fn(params, model, X_P, X_H, X_rho, y):
    model = eqx.combine(params, model)
    y_pred = jax.vmap(model)(X_P, X_H, X_rho)
    return jnp.mean((y_pred - y)**2)

@eqx.filter_jit
def step(params, model, opt_state, X_P, X_H, X_rho, y):
    loss, grads = loss_fn(params, model, X_P, X_H, X_rho, y)
    updates, opt_state = opt.update(grads, opt_state)
    params = optax.apply_updates(params, updates)
    return params, opt_state, loss

# --- Init model ---
save_path = "/srv/scratch3/taylor.4264/odd_emu/models_final_bacco"
...
for epoch in range(n_epochs):
    ...
    for i in range(n_batches):
        s = i * batch_size
        e = s + batch_size
        batch = (X_P_train[s:e], X_H_train[s:e], X_rho_train[s:e], y_train[s:e])
        params, opt_state, batch_loss = step(params, model, opt_state, *batch)
        epoch_loss += batch_loss
    epoch_loss /= n_batches

    val_loss, _ = loss_fn(params, model, X_P_val, X_H_val, X_rho_val, y_val)
...
eqx.tree_serialise_leaves(f"{save_path}/learned_model_logpk_noz_{run_idx}.eqx", best_params)
