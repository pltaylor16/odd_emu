...
# --- Load merged dataset ---
data = np.load('/srv/scratch3/taylor.4264/odd_emu/production_run_logpk/merged/logpk_data_with_dHz_dz.npz')
logpk = jnp.array(data["logpk"])
logpk_dz = jnp.array(data["logpk_dz"])
Hz = jnp.array(data["Hz"])
dHz_dz = jnp.array(data["dHz_dz"])
...
dHz_dz_norm = (dHz_dz - dHz_dz.mean()) / dHz_dz.std()

X_H = Hz_norm.reshape(-1, 1)
X_rho = rho_norm.reshape(-1, 1)
X_dH = dHz_dz_norm.reshape(-1, 1)
...
X_dH_train, X_dH_val = X_dH[:split_idx], X_dH[split_idx:]
...
class RHS(eqx.Module):
    ...
    def __call__(self, P, H, rho, dH):
        return self.mlp(jnp.concatenate([P, H, rho, dH]))

@eqx.filter_value_and_grad
def loss_fn(params, model, X_P, X_H, X_rho, X_dH, y):
    model = eqx.combine(params, model)
    y_pred = jax.vmap(model)(X_P, X_H, X_rho, X_dH)
    return jnp.mean((y_pred - y)**2)

@eqx.filter_jit
def step(params, model, opt_state, X_P, X_H, X_rho, X_dH, y):
    loss, grads = loss_fn(params, model, X_P, X_H, X_rho, X_dH, y)
...
for epoch in range(n_epochs):
    ...
    val_loss, _ = loss_fn(params, model, X_P_val, X_H_val, X_rho_val, X_dH_val, y_val)
...
eqx.tree_serialise_leaves(f"{save_path}/learned_model_logpk_dH_{run_idx}.eqx", best_params)
