...
k_idx = np.where((k_emulator > 5e-2) & (k_emulator < 2.))[0][::6]
k = k_emulator[k_idx]
k_shape = len(k)
...
for i in range(n_local):
    ...
    _, boost_dict = ee2.get_boost(cosmo_dict, z_array, k_h_units)
    ...
    for j, z in enumerate(z_array):
        ...
        _, bm = ee2.get_boost(cosmo_dict, [z_minus], k_h_units)
        _, bp = ee2.get_boost(cosmo_dict, [z_plus], k_h_units)
